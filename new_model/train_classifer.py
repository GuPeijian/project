...
def padding_classifer_fn(datalist):
    ...
    for i,item in enumerate(datalist):
        pad_len=max_comment_len-comment_length[i]
        comment=item["comment"]
        #开头改为[CLS],即101
        comment[0]=101
        attention_mask=[1]*comment_length[i]
        #[PAD]为0
        comment.extend([0]*pad_len)
        attention_mask.extend([0]*pad_len)

        input_ids.append(comment)
...
def main():
    ...
    # data and model
    parser.add_argument("--data_path", default='./processd_dataset_v2.json', type=str, )
    parser.add_argument("--model_name_or_path", default="/home/gpj/bert-chinese", type=str, )
    parser.add_argument("--output_dir", default='./sentiment_classifer1', type=str, )
    ...
    train_dataloader = DataLoader(train_dataset, num_workers=0, batch_size=args.batch_size, shuffle=False,
                                  drop_last=True, collate_fn=padding_classifer_fn, sampler=train_sampler)
    ...
    tb_writer = SummaryWriter('./classifer_loss1')
