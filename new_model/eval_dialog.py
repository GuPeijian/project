...
model_path = "./sentiment_output_bow_clean/"
...
input_text7 = "【直播！#暑期如何做好近视防控#？】暑假是孩子近视高发期，家长该如何打好近视防控战？孩子看手机时间长，有什么护眼好方法？OK镜、防蓝光镜有用吗？#央视新闻公开课#邀请北京大学人民医院眼视光中心主任医师王凯，为近视防控支招！（总台央视记者顾小慈）"
text_id = tokenizer.encode(input_text5, max_length=250, truncation=True, add_special_tokens=True)

user_id = [tokenizer.convert_tokens_to_ids('<user1>'), tokenizer.convert_tokens_to_ids('<user2>')]
input_ids_list = text_id + [user_id[0]]
...
a = model.generate(input_ids, topp=0.96, no_repeat_ngram_size=3, do_sample=True, max_length=text_len+40, min_length=text_len+7,
                       early_stopping=True, temperature=0.8,
                       sentiment_input=sentiment)
c=tokenizer.decode(a[0][text_len:])
...
user_id_2=torch.tensor([[user_id[1]]]).to(device)

input_ids=torch.cat((a,user_id_2),dim=1)
