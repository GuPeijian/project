...
def main():
    ...
    #data and model
    parser.add_argument("--data_path",default='./processd_dataset.json',type=str,)
    parser.add_argument("--model_name_or_path",default='/home/gpj/bert-chinese',type=str,)
    parser.add_argument("--output_dir",default='./sentiment_output_bow_clean',type=str,)
    ...
    train_dataloader = DataLoader(train_dataset,num_workers=0,batch_size=args.batch_size, shuffle=False, drop_last=True, collate_fn=padding_fuse_fn,sampler=train_sampler)
    ...
    tb_writer = SummaryWriter('./sentiment_loss')
    ...
    num_train_steps=math.floor((train_dataset.__len__())/(args.batch_size*args.gpus*args.gradient_accumulation_steps))*args.num_train_epochs
    ...
    for epoch in trange(int(args.num_train_epochs),desc='Epoch'):
        for step,batch in enumerate(tqdm(train_dataloader,desc='Iteration')):
            input_ids, attention_mask, token_type_ids, labels , sentiment= \
                batch["input_ids"], batch["attention_mask"], batch["token_type_ids"], batch["labels"], batch["sentiment"]
