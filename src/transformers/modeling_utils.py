...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        #double_model=kwargs.pop("double_model",False)
        ...
        if from_tf:
            ...
            if double_model:
                dict1=state_dict
                dict2=state_dict.copy()
                for item in list(dict1.keys()):
                    new_name=item.replace('bert','bert.encoder')
                    dict1[new_name]=dict1.pop(item)
                for item in list(dict2.keys()):
                    new_name=item.replace('bert','bert.decoder')
                    dict2[new_name]=dict2.pop(item)
                dict1.update(dict2)
                state_dict=dict1.copy()
                del dict1
                del dict2
            '''
